...
def flair_Embedding(inputs):
    ...
    # allinput = torch.tensor((allinput), device=cuda0)
    allinput = torch.tensor(allinput)
    allinput = allinput.to(torch.float32)
    for i in range(len(inputs)):
        ...
        sen = []
        # cuda0 = torch.device('cuda:0')
        # sen = torch.tensor((sen), device=cuda0)
        sen = torch.tensor(sen)
        sen = sen.to(torch.float32)
        for token in sentence:
            emb = token.embedding
            emb = emb.reshape(-1, 868)
            # emb = emb.reshape(-1, 1100)
            if sen.numel() == 0:
                sen = emb
            else:
                sen = torch.cat((sen, emb), dim=0)
        if allinput.numel() == 0:
            allinput = sen
        else:
            # allinput = torch.stack((allinput, sen), dim=0)
            allinput = torch.cat((allinput, sen), dim=0)
    out = allinput.reshape(-1, 500, 868)
    # out = allinput.reshape(-1, 500, 1100)
    # nn_emb = torch.tensor(nn_emb)
    # nn_out = torch.cat((nn_emb, out), dim=2)
    return out
...
class FastMLAttention(nn.Module):
    ...
    def forward(self, inputs, masks, candidates, attn_weights: nn.Module):
        ...
        attention = (attn_weights @ attn_inputs).masked_fill(~masks, -np.inf)
